...
class addevent implements ActionListener{
	...
	addevent(){
		...
		jcb2.setSize(150,30);
		jcb2.addItem("获取一个随机数");         //0 
		jcb2.addItem("关机");                  //0 
		jcb2.addItem("文字提示");               //1
		jcb2.addItem("文件内容提示");           //1   
		jcb2.addItem("打开一个文件");           //1
		jcb2.addItem("命令行指令");             //1
		jcb2.addItem("关闭某个程序");           //1
		jcb2.addItem("向文件写入内容");         //2
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==jbt1){
			try{
				...
				if(b){
					switch((String)(jcb2.getSelectedItem())){
					case "获取一个随机数":
					case "关机":
						n=true;
						break;
					...
					case "命令行指令":
					case "关闭某个程序":{
						if(!(jtf2.getText().equals(s.toString()))){
							n=true;
						}
						break;
					}
					case "向文件写入内容":{
						if(!(jtf2.getText().equals(s.toString()))&&!(jtf3.getText().equals(s.toString()))){
							n=true;
						}
						break;
					}
		...
		if(e.getSource()==jcb2){
			...
			case "向文件写入内容":
				...
				break;
			case "关闭某个程序":
				jl5.setText("参数为 要关闭的程序名(程序的进程名)");
				jl7.setText("无需参数");
				break;
			default:
				jl5.setText("无需参数");
