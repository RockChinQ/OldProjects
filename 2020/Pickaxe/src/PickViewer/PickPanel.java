	...
	public void loadPickImage(int x,int y,PickImage pi) {
		...
		for(int i=0;i<pi.h;i++) {
			for(int j=0;j<pi.w;j++) {
				if(i+y>=this.gridOfHeight||j+x>=this.gridOfWidth||i+y<0||j+x<0) {
					index++;
					continue;
				}
